...
class ZlifeSupabaseUser extends BaseAuthUser {
  ZlifeSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> zlifeSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = ZlifeSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
